...
int main() {
  ...
  nav::Planner planner;
  try {
    planner = nav::Planner(nav_map_xlen, nav_map_ylen, nav_map_zlen, nav_map_nx,
  ...
  {
    std::ofstream ofs("../data/planner.dat");
  ...
  try {
    empty_planner = nav::Planner(nav_map_xlen, nav_map_ylen, nav_map_zlen,
                                 nav_map_nx, nav_map_ny, nav_map_nz,
                                 drone_radius, drone_height, empty_pntcloud);
  } catch (const char *msg) {
    std::cerr << msg << std::endl;
    exit(EXIT_FAILURE);
  }
  {
    std::ofstream ofs("../data/empty_planner.dat");
    boost::archive::binary_oarchive oa(ofs);
    oa << empty_planner;
  }
